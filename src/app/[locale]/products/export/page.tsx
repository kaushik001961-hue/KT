...
// Map to bundle category with a representative thumbnail image from its products
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // Extract unique categories and grab a small image thumbnail from the first matching product
        if (!categoryId) {
          ...
          fetchedProducts.forEach((product: Product) => {
            if (product.category) {
              ...
              if (!catMap.has(catId)) {
                // Find first available image from product to use as small category thumbnail
          ...
          <div className="mx-auto max-w-4xl text-center">
            ...
            <h1 className="mt-4 text-4xl font-bold leading-[1.05] tracking-tight text-[var(--foreground)] sm:text-5xl lg:text-6xl">
              <span className="gradient-text">
                {categoryId ? "Explore category products." : "Explore our export categories."}
              </span>
            </h1>

            <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-[var(--foreground)]/65 sm:text-lg">
              {categoryId
                ? "Showing products available under this selected export category."
                : "Select a category below to browse published export products and international market opportunities."}
            </p>
          </div>
        </div>
      </section>
          ...
          {!loading && !error && !categoryId && categories.length > 0 && (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {categories.map((cat) => (
                <Link
                  key={cat.id}
                  href={`/products/export?categoryId=${cat.id}`}
          ...
          {!loading && !error && ((categoryId && products.length === 0) || (!categoryId && categories.length === 0)) && (
            <div className="mx-auto max-w-2xl rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-10 text-center shadow-sm sm:p-14">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--primary-light)]">
                <Globe2 size={30} className="text-[#1455a0] dark:text-[#68b0ff]" />
              </div>
              <h2 className="mt-6 text-2xl font-bold text-[var(--foreground)]">
                {categoryId ? "Currently no product available for this category" : "No export categories available"}
              </h2>
              <p className="mt-3 leading-7 text-[var(--foreground)]/60">
                {categoryId
                  ? "There are currently no published products available in this category."
                  : "Our export product catalogue is currently being updated."}
              </p>
              <Link
                href="/contact"
