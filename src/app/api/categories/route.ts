...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existing = await prisma.category.findUnique({ where: { slug_type: { slug, type } } });
    if (existing) {
      return NextResponse.json({ message: "A category with this name already exists for this type." }, { status: 409 });
    }

    const category = await prisma.category.create({
      data: { name, slug, type, description, imageUrl, active: true },
    });
